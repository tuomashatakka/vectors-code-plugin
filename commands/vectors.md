...
- If the query starts with `all:`, strip that prefix and call `search_global`
  with the rest — search across every project (optionally pass a `projects`
  comma-list to scope to a subset).
...
- Summarize the top hits with their `project`, `title` (AST chunks are titled
  `path › symbol`), `unit_type`, `url`, and the relevant snippet. Note the
  result's `confidence` tier. Prefer quoting the stored chunk text over guessing.

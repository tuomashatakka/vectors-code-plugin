...
@mcp.tool()
def search(query: str, project: str = "", topk: int = 8, rerank: bool = True) -> dict:
    """Semantic search within ONE project. `project` defaults to the one
    resolved from the working directory. Returns reranked chunks with project,
    source, title, url, snippet and scores."""
    name = _resolve(project)
    return _strip(vi.Project.load(name).search(query, topk=topk, rerank=rerank))


@mcp.tool()
def search_global(query: str, topk: int = 8, rerank: bool = True, projects: str = "") -> dict:
    """Semantic search across EVERY project in the global store (or a comma-
    separated subset via `projects`). Each hit is tagged with its project. This
    is the right tool when you don't know which project holds the answer, or
    when the server has no project-specific working directory."""

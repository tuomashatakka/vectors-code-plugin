...
| `viewer/` | `server.ts` (live 3D synapse viewer HTTP + JSON API — `/api/status`, `/api/inventory`, `/api/doc`, `/api/node`, `/api/graph`, `/api/search`, `/api/intents`, `/api/events` SSE, `/api/projects`; `?project=*` all-projects scope, full-document `/api/doc?full=1`), `static.ts` (traversal-safe static-asset serving for the split front-end bundle, ETag/304, `/vendor/three/*`), `graph.ts` (per-project PCA clusters, knn links, golden-spiral layout, retained projection state). Front-end bundle lives at `assets/viewer/` (`index.html` + `viewer.css` + ES modules under `js/`), mirrored into `skills/vector-index/assets/viewer/` by `scripts/sync-assets.ts` (`bun run assets:sync`). |
